...
function t_animate__init() {
    if (1 == window.isSearchBot || "yes" == $(".t-records").attr("data-blocks-animationoff") || t_animate__checkIE() || "edit" == $(".t-records").attr("data-tilda-mode")) $(".t-animate").removeClass("t-animate");
        ...
        1200 <= $(window).width() && t_animate__parseNumberText(t), setTimeout(function () {
...
function t_animate__startAnimation() {
    ...
    $(window).width() < 1200 && (t = t_animate__checkMobile(t), a = t_animate__checkMobile(a), e = t_animate__checkMobile(e)), (void 0 !== t && 0 != t.length || void 0 !== e && 0 != e.length || void 0 !== a && 0 != a.length) && (t_animate__setAnimationState(t, a, e), t = t.filter(".r:has(.t-animate_wait)"), e = e.filter(".t-animate_wait"), a = a.filter(".r:has(.t-animate_wait)"), n(), $(window).bind("resize", t_throttle(n, 200)), setInterval(n, 5e3), $(window).bind("scroll", t_throttle(function () {
...
function t_animate__animateChainItemsOnScroll(t, a, e) {
    var n = $(t[a]),
        ...
        m = t_animate__getDelayFromPreviousScrollEvent(n, i, .16),
        _ = t_animate__getSectionHeadDealy(n);
    $(i[0]).addClass("t-animate__chain_first-in-row");
    for (var l = 0; l < i.length; l++) {
        var d = $(i[l]),
            c = t[a].itemsOffsets[l];
        if (!(c < e)) break;
        c != o && (d.addClass("t-animate__chain_first-in-row"), s = ++r, o = c);
        var f = .16 * s + m + _;
        d.css("transition-delay", f + "s"), d.removeClass("t-animate_wait"), d.addClass("t-animate_started"), d.attr("data-animate-start-time", Date.now() + 1e3 * f), d[0] == i.last()[0] && t_animate__checkSectionButtonAnimation(n, f), c == o && s++, i.splice(l, 1), t[a].itemsOffsets.splice(l, 1), l--
    }
...
function t_animate__getSectionHeadDealy(t) {
    var a = t.find(".t-section__title.t-animate"),
        ...
        n = 0;
    return a.length && Date.now() - a.attr("data-animate-start-time") <= 160 || e.length && Date.now() - e.attr("data-animate-start-time") <= 160 ? n = .16 : n
}
    ...
    t.each(function () {
        ...
        if (0 !== t.find('div[data-customstyle="yes"]').length) {
            ...
            t.find("span").each(function () {
                e += $(this).attr("style"), $(this).removeAttr("style"), $(this).removeAttr("data-redactor-style")
            }), a = t.find('div[data-customstyle="yes"]').html();
            var n = t.attr("style");
        ...
        if (!($(this).offset().top < o - 500) && 0 < a.length) {
            var i = a.match(/\d+\.\d+|\d+\,\d+/g),
                s = a.replace(/(\d)(?= \d) /g, "$1"),
                r = [];
            s.split(" ").forEach(function (t, a) {
                isNaN(parseInt(t)) || r.push(t)
            }), r.forEach(function (t) {
                var a = s.indexOf(t),
                    e = s.substr(a, t.length);
                s = s.replace(e, "num")
            }), r && (r.forEach(function (t, a) {
                var e;
                if (null !== i && (-1 !== t.indexOf(",") && (e = t.split(",")), -1 !== t.indexOf(".") && (e = t.split(".")), -1 !== t.indexOf(",") || -1 !== t.indexOf("."))) {
                    var n = e[1].length;
                    r[a] = +e.join("."), r[a] = r[a].toFixed(n)
                }
            }), t.attr("data-animate-number-count", a), r.forEach(function () {
                t_animate__changeNumberOnZero(t, s)
            }), t.find("span:not(.t-animate__number):first").each(function () {
                $(this).attr("style", e)
            }))
        }
    })
...
function t_animate__changeNumberOnZero(t, a) {
    var e;
    e = a.replace(/num/g, '<span class="t-animate__number">0</span>'), t.html(e)
}

function t_animate__animateNumbers(t) {
    t.each(function () {
        var s = $(this),
            a = s.attr("data-animate-number-count"),
            e = [];
        if (s.find("span:not(.t-animate__number):first").each(function () {
                e = $(this).attr("style")
            }), a) {
            var n = a.match(/\d+\.\d+|\d+\,\d+/g),
                r = a.match(/\d+/g),
                t = a.replace(/(\d)(?= \d) /g, "$1").split(" "),
                i = [];
            t.forEach(function (t, a) {
                isNaN(parseInt(t)) || i.push(t)
            });
            var o = 0,
                m = !1;
            s.removeAttr("data-animate-number-count"), null != n && (m = -1 != n[0].indexOf(",")), i.forEach(function (t, a) {
                var e;
                null !== n && (-1 !== t.indexOf(",") && (e = t.split(",")), -1 !== t.indexOf(".") && (e = t.split(".")), -1 === t.indexOf(",") && -1 === t.indexOf(".") || (o = e[1].length, i[a] = +e.join("."), 0))
            });
            var _ = [];
            s.find(".t-animate__number").each(function () {
                _.push($(this).text())
                ...
                $({
                    ...
                    easing: "swing",
                    step: function (t) {
                        var a = s.find(".t-animate__number")[i],
                            e = Math.pow(10, o),
                            n = t ? (Math.round(this.animateCounter * e) / e).toFixed(o) + "" : Math.floor(this.animateCounter) + "";
                        n = 1 < r.length ? n.replace(/(\d)(?=(\d{3})+([^\d]|$))/g, "$1 ") : n, m ? $(a).text(n.replace(/\./g, ",")) : $(a).text(n)
                    },
                    complete: function () {
                        s.html(a), s.find("span").each(function () {
                            $(this).attr("style", e)
                        })
                    }
                })
...
function t_animate__assignChainDelay(n, i, s) {
    ...
    if (0 != r.length) {
        ...
        r.each(function () {
            ...
            if (a < i) {
                a != m && (t.addClass("t-animate__chain_first-in-row"), m = a);
                var e = .16 * o + _;
                t.css("transition-delay", e + "s"), t.addClass("t-animate_started"), t.attr("data-animate-start-time", Date.now() + 1e3 * e), t[0] == r.last()[0] && t_animate__checkSectionButtonAnimation(n, e), o++, t.on("TransitionEnd webkitTransitionEnd oTransitionEnd MSTransitionEnd", function (t) {
                    $(this).addClass("t-animate__chain_showed"), $(this).off(t)
                })
...
function t_animate__detectElemTriggerOffset(t, a) {
    var e = t_animate__getAttrByRes(t, "trigger-offset"),
        n = a;
    return void 0 !== e && "" != e && (n = a - (e = e.replace("px", ""))), n
}
...
function t_animate__getGroupsOffsets(t) {
    for (var a = 0; a < t.length; a++) 0 < $(t[a]).find(".t-animate").length && (t[a].curTopOffset = $(t[a].querySelector(".t-animate")).offset().top)
}
...
function t_animate__wrapTextWithOpacity() {
    $(".t-title.t-animate,.t-descr.t-animate,.t-uptitle.t-animate,.t-text.t-animate").each(function () {
        ...
        if (void 0 !== a && 0 <= a.indexOf("opacity")) {
            ...
            if (0 <= a.slice(e).indexOf(";")) var n = a.slice(e, e + a.slice(e).indexOf(";"));
            else n = a.slice(e);
